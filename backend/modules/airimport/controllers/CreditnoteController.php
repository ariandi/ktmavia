...
use backend\modules\airimport\models\CreditnoteSearch;
...
class CreditnoteController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            return $this->redirect(['view', 'id' => $model->criditnote_id]);
        } else {
    ...
    public function actionCreditnoteview($id)
    {
        $this->layout = 'blankpage';

        $model      = $this->findModel($id);
        $modeldet   = Creditnotedet::findAll(['creditnote_id' => $id]);
        ...
        $mpdf = new mPDF;
        $mpdf->WriteHTML(
                            $this->renderPartial('creditnotepdfview', ['model' => $model, 'modeldet' => $modeldet, 'terbilang' => $terbilang])
                        );
        $mpdf->Output();
        exit;    
    }
