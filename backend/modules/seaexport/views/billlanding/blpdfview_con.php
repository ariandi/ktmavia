...
<style type="text/css">
	...
	.head1 .htext2{font-size: 12px;font-weight: bold;float: left; width: 200px;margin-left: 10px;margin-top: 3px;}
...
<div class="wrapperpdf">
	...
	<div class="head1">
		...
		<div class="htext2">Attachment - <?=$model->bl_number?></div>
	</div>

	<table border="1" style="width:96%;font-size:12px;">
		<thead>
			<tr>
				<th>No.</th>
				<th width="100">Container Number</th>
				<th>Seal Number</th>
		...
		<tbody>
			...
			foreach ($modeldet as $modeldet) {
				...
				if($i == 1){
					$getfirst_w = $modeldet->weight;
					$ex = explode(" / ", $getfirst_w);
				}

				if($i !== 1){
					
					//echo $modeldet->container_seal_no.'<br />';
					$pecah = explode(' / ', $modeldet->container_seal_no)
			?>
					<tr>
						<td><?=$j?></td>
						<td><?=$pecah[0]?></td>
